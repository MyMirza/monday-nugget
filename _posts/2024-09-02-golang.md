...
categories: [golang]
---
Welcome to this week's edition of "GoLang Nugget," where we dive into the latest insights and tips for Go enthusiasts. We start with a performance comparison between gRPC with Protocol Buffers and HTTP with JSON, revealing that HTTP/1 surprisingly outperformed gRPC in speed tests. Next, we explore building static binaries in Go on Linux, highlighting the use of build tags and alternative compilers like musl and Zig for static linking. We then delve into hash-based bisect debugging, a tool that simplifies pinpointing bugs in complex codebases. The new `unique` package in Go 1.23 is introduced, offering efficient value deduplication. For those building REST APIs, we cover using the Ent framework with `net/http` to streamline development. We also discuss implementing graceful shutdowns in Go applications on Kubernetes to ensure data integrity during updates. Lastly, we look at a method for testing helper functions within structs by injecting fake functions, simplifying the testing process. Enjoy these nuggets of Go wisdom and happy coding!
